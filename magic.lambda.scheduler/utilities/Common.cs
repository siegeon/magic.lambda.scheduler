...
namespace magic.lambda.scheduler.utilities
{
    ...
    public static class Common
    {
        ...
        static readonly string _fileHeader = @"/*
 * WARNING!
        ...
        public static void Initialize(IServiceProvider provider, string tasksFile)
        {
            lock (_initLocker)
            {
                ...
                if (!File.Exists(tasksFile))
                {
                    ...
                    {
                        // Making sure we write file's header (the warning parts).
                        stream.Write(_fileHeader);
                    }
                }
                else
                {
                    ...
                    using (var stream = File.OpenRead(tasksFile))
                    {
                        ...
                        _tasks.Write(tasks =>
                        {
                            ...
                            {
                                // Creating our task from its serialized declaration.
                                var idxTask = Task.CreateFromSerialized(provider, idx);

                                /*
                                 * Notice, sometimes the date for the currently iterated timer has passed,
                                 * at which point we do NOT add it to the tasks, but simply ignore it.
                                 */
                                if (idxTask != null)
                                    tasks.Add(idxTask);
                            }
