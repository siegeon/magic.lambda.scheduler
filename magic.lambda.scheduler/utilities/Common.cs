...
namespace magic.lambda.scheduler.utilities
{
    ...
    public static class Common
    {
        ...
        public static void Initialize(string rootPath)
        {
            ...
            if (TasksFolder != null)
                throw new ApplicationException($"You can't invoke {nameof(Initialize)} twice for the task scheduler.");
...
");
         ...
         */
        internal static void AddTask(ISignaler signaler, Node task)
        {
            ...
            _tasks.Write((tasks) =>
            {
                ...
                signaler.Signal("lambda2hyper", lambda);
                using (var writer = File.CreateText(TasksFolder + taskName + ".hl"))
                {
                    writer.Write(lambda.GetEx<string>());
                }
                ...
                var clonedTasks = tasks.Clone();
                signaler.Signal("lambda2hyper", clonedTasks);
                using (var writer = File.CreateText(TasksFile))
                {
                    writer.Write(clonedTasks.GetEx<string>());
                }
            });
        }
